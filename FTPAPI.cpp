...
SOCKET connect_server(char *host, int port)
{
    ...
    if/*while*/((len = recv(ctrl_sock, buf, BUFSIZE, 0)) > 0)
    {
    ...
    if(FTP_SERVICE_READY != result)//220
    {
 ...
 * */
SOCKET ftp_pasv_connect(char* ip,SOCKET c_sock)
{
    SOCKET r_sock;
    int send_result;
    ssize_t len;
    ...
    r_sock = socket_connect(ip,addr[4]*256+addr[5]);
    if(-1 == r_sock)
        return -1;
    return r_sock;
}
 ...
 * */
int ftp_list(char* ip,SOCKET c_sock, char *path, char **data, int *data_len)
{
    SOCKET r_sock;
    char buf[BUFSIZE];
    int send_re;
    ...
    //连接到PASV接口
    r_sock = ftp_pasv_connect(ip,c_sock);
    if(-1 == r_sock)
    {
        return -1;
    }
 ...
 *         -1:文件创建失败  -2 pasv接口错误
 * */
int ftp_server2local(char *ip,SOCKET c_sock, char *s, char *d, int * size)
{
    ...
    *size=0;
    ...
    //连接到PASV接口 用于传输文件
    d_sock = ftp_pasv_connect(ip,c_sock);
    if(-1 == (int)d_sock)
    {
		MessageBox(NULL, _T("设置消极模式失败"), NULL, 0);
        fclose(fp); //关闭文件
        return -2;
    }
    ...
    {
		MessageBox(NULL, _T("下载命令失败"), NULL, 0);
        fclose(fp);
        return -1;
    }
    ...
    {
		TCHAR eroNo[100];
		_itot(result, eroNo, 10);
		MessageBox(NULL, _T("下载失败"), eroNo, 0);
        return -1;
    }
 ...
 *         -1:文件创建失败  -2 pasv接口错误
 * */
int ftp_local2server(char* ip,SOCKET c_sock, char *s, char *d, int * size)
{
    SOCKET d_sock;
