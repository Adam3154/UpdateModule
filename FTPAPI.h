...
SOCKET ftp_pasv_connect(char* ip,SOCKET c_sock); //连接到PASV接口
int ftp_list(char* ip,SOCKET c_sock, char *path, char **data, int *data_len); //列出FTP工作空间的所有目录
...
int ftp_server2local(char* ip,SOCKET c_sock, char *s, char *d, int * size); //从服务器复制文件到本地 RETR
int ftp_local2server(char* ip,SOCKET c_sock, char *s, char *d, int * size); //从本地复制文件到服务器 STOR
int ftp_recv(SOCKET sock, char *re_buf, ssize_t *len); //获取响应码
